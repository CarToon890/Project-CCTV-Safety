...
- **Audit Execution Outcome (25 September 2026):** 100% machine inventory completed (21,527 images/labels; raw fire: 14,692, raw smoke: 11,865; 0 corrupt, 0 syntax errors). 26 OOB bounding box defects resolved (18 zero-dimension dropped, 379 clipped to bounds). Upstream split leakage diagnosed (762 upstream leaking groups affecting 10,504 images across splits) and resolved via sequence/dHash clustering into 10,010 scene groups (0 cross-split leakage; splits: train 17,248 / val 1,488 / test 2,791; raw splits: train 14,122 / val 3,099 / test 4,306). 106-frame worker visual sample audit executed; corrected dataset built at `data/processed/dfire_corrected` (14,685 fire boxes, 11,854 smoke boxes); handoff queue emitted for all 106 rows in `dfire_audit_handoff_queue.csv`.
- **Decision:** **CONDITIONAL GO — PASSED WORKER SAMPLE AUDIT (Corrected Build Ready; Hold for Independent Human QA).** Authorizes bounded educational prototype evaluation only; **never unrestricted training, public weight release, commercial deployment, or clean provenance claims**. Must satisfy strict safeguards: raw images excluded from Git, model weights private, source cited, identifiable faces blurred, and upstream-rights limitations disclosed in reports/presentations. Independent human QA sign-off across all 106 rows remains final approval gate. See [fall_fire_replacement_dataset_search.md](fall_fire_replacement_dataset_search.md).
