...
> **Physical Data Presence Disclosure:** Raw image/video archives for Fall, Smoke, and Fire candidates are **absent locally from disk** (`data/raw/` does not contain them), and their actual physical sample audits have **not been executed**. Sample audit protocols and sample size inventories have been designed and approved via primary-source desk reviews, but awaiting physical acquisition upon owner authorization.
...
- **Completed Evidence:**
  ...
  - Decision-complete 10-clip sample audit protocol designed (Section 5 of replacement search doc).
- **Current Limitations & Unfinished Work:**
  - **Archive absent locally:** Dataset video files have not been downloaded to `data/raw/fall/`.
  - **Sample audit unexecuted:** Physical extraction of 410–700 frames, verification of `mapped_auto_exact_frame_count`, and bounding-box precision checks have not been performed on disk.
  - **Dual-box semantics:** Verifying that fallen individuals possess co-occurring `person` (0) and `fall` (3) bounding boxes requires visual verification once files are acquired.
